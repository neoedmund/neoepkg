...
public class Config {
	...
	public static Map getConfig(String cfgName, boolean needComma)
			throws Exception {
	 ...
	 * @param needComma
	 */
	private static synchronized Map initInFileSystem(String cfgName,
			boolean needComma) throws Exception {
		File f1 = new File(cfgName);
		Log.log("load config file:" + f1.getAbsolutePath());
		FileInputStream in;
		Map config = (Map) PyData.parseAll(
				FileUtil.readString(in = new FileInputStream(f1), "UTF8"),
				needComma);
		in.close();
